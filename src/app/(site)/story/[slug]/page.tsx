...
import { getMeta, getStory } from "@/data";
...
export default async function StoryPage({ params }: Props) {
  const { slug } = await params;
  const [story, meta] = await Promise.all([getStory(slug), getMeta()]);
  if (!story) notFound();
  const topic = story.topic;
  const backHref = story.date === meta.currentDate ? "/" : `/d/${story.date}`;
  ...
  return (
    <article>
      <Link className="back press" href={backHref}>
        <ArrowLeftIcon />
        <T
          zh={story.date === meta.currentDate ? "返回今日早报" : `返回 ${backDate.zh}早报`}
          en={story.date === meta.currentDate ? "Back to today’s briefing" : `Back to ${backDate.en}`}
        />
      </Link>
